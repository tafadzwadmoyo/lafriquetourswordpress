...
function rednao_get_fixed_field_value($match,$entryData,$elementOptions)
{
	...
	try{
		$value=apply_filters("smart-forms-fixed-field-value-".$fixedFieldParameters["Op"],$fixedFieldParameters,$entryData,$elementOptions);
...
function GetValueByField($stringBuilder,$match,$entryData,$elementOptions,$useTestData)
{
	if(strpos(trim($match),'{')===0)
	{
        if($useTestData)
            return 'Sample data';
		return rednao_get_fixed_field_value($match,$entryData,$elementOptions);
	}
